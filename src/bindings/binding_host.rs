use druid::{BoxConstraints, Command, Data, Env, Event, EventCtx, ExtEventSink, LayoutCtx, LifeCycle, LifeCycleCtx, PaintCtx, Selector, Size, UpdateCtx, Widget, WidgetId};
use std::marker::PhantomData;
use crate::bindings::{BindableAccess, Binding};
    ...
    > BindingHost<T, U, Contained, Controlled, B>
{
    ...
    fn check_for_changes2(&mut self, data: &T, env: &Env, ctx: ExtEventSink, id: WidgetId) {
        if let BindingHostState::TwoWay = self.state {
            ...
            if self.pending_change.is_some() {
                ctx.submit_command(APPLY_BINDINGS, (), id);
            }
        }
    }
    ...
    fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut OuterData, env: &Env) {
        match self.state {
            ...
            BindingHostState::TwoWay => {
                ...
                self.check_for_changes2(data, env, ctx.get_external_handle(), ctx.widget_id());
            }
        }
    }
